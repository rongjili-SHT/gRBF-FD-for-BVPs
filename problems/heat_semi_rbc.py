...
def precompute_schur_solver_bdf4(L, D_n, id_interior, id_boundary, N, dt, nu):
    ...
    # ### debug
    # L_eff = L_II - L_IB @ B_BB_inv @ B_BI
    
    # eigenvalues, _ = splinalg.eigs(L_eff, k=5, sigma=1e2, which='LM')
    # print(f"L_eff max eigenvalues: {np.real(eigenvalues)}")
    # ### debug
...
def heat_semi_rbc(N=6400, l=4, K=30, l_grad=4, K_grad=35, lap_opt='qp', dn_opt='qp', seed=None, vis=False, dill_file='./data/rbc_mms_funcs.dill', max_retires_opt=60):
    #-- PARAMETERS --#
    kappa = 3
    delta = 1e-5

    # print(f"N = {N} l_in = {l} l_bd = {l_grad}")
    ...
    u_vals, f_vals, u_lap_vals, u_grad_vals, g_vals = compute_mms_rbc(manifold, n_vecs, cache_file=dill_file)
...
if __name__ == "__main__":
    ie_l2 = heat_semi_rbc(N=25600, l=4, K=25, l_grad=4, K_grad=30, seed=0)